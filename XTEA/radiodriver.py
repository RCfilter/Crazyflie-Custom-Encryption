...
key_py = [int('1b4f9d87', 16), int('016510fd', 16), int('abcd16af', 16), int('e96328d5', 16)]
...
class RadioDriver(CRTPDriver):
    ...
    def receive_packet(self, timee=0):
        ...
        if len(t) > 24:
            d = t[24:]
            t = t[:24]
        if len(t)%24 == 0 and encrypt:
            t1 = (ctypes.c_ubyte*8)(*t[:8])
            t2 = (ctypes.c_ubyte*8)(*t[8:16])
            t3 = (ctypes.c_ubyte*8)(*t[16:])
            tea.XTeaDecrypt(t1, key)
            tea.XTeaDecrypt(t2, key)
            tea.XTeaDecrypt(t3, key)
    ...
    def send_packet(self, pk):
        ...
        if encrypt:
            ...
            t2 = (ctypes.c_ubyte*8)(*t[8:16])
            t3 = (ctypes.c_ubyte*8)(*t[16:])
            tea.XTeaEncrypt(t1, key)
            tea.XTeaEncrypt(t2, key)
            tea.XTeaEncrypt(t3, key)
            dec = list(t1) + list(t2) + list(t3)
            pk._data = bytearray(dec) + d
            #print('\n...Sending...\n')
            #print(pk._data)
