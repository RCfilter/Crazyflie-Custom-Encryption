...
import sys
import threading
import blowfish as bf #sudo pip3 install blowfish
...
key = bytes.fromhex('1b4f9d87016510fdabcd16afe96328d5')
cipher = bf.Cipher(key, byte_order = "little")
...
class RadioDriver(CRTPDriver):
    ...
    def receive_packet(self, time=0):
        ...
        if len(t) > 16:
            d = t[16:]
            t = t[:16]
        if len(t)%8 == 0 and encrypt:
            dec = b"".join(cipher.decrypt_ecb(t))
            pk._data = dec + d
        else:
            dec = t
        pk._data = dec + d
    ...
    def send_packet(self, pk):
        ...
        if encrypt:
            ...
            text = text[1:] + bytes([text[0]])
            enc = b"".join(cipher.encrypt_ecb(text))
            pk._data = enc + d
        else:
            pk._data = t + d
        print('\n...Sending...')
